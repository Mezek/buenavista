 ...
 * $Revision: 349 $
 * $Author: bartos $
 * $HeadURL: http://triglav/repos/BuenaVista/Hyperons/NucleonUamNaive.cpp $
 * $Id: NucleonUamNaive.cpp 349 2013-05-22 08:09:53Z bartos $
 *
 * @file
 * @brief	UA form factor for nucleons - naive model
 */
...
void FFactor::FixParameters ()
{
	...
	val[8] = 1.;
	val[9] = 1.;
	std::cout << "> Parameters val[5], val[6], val[8], val[9] fixed!" << std::endl;
}
...
TComplex FFactor::ScalarOne (TComplex t)
{
	...
	suma = normA*((mul[0]-mul[1])*a[4] + mul[1]*nor[0]);
	return suma;
}
...
TComplex FFactor::ScalarTwo (TComplex t)
{
	...
	suma = normA*((mul[0]-mul[1])*a[7] + mul[1]*nor[1]);
	return suma;
}
