 ...
 * $Revision: 349 $
 * $Author: bartos $
 * $HeadURL: http://triglav/repos/BuenaVista/Hyperons/NucleonUamHybrid.cpp $
 * $Id: NucleonUamHybrid.cpp 349 2013-05-22 08:09:53Z bartos $
 *
 * @file
 * @brief	UA form factor for nucleons - hybrid model
 */
...
void FFactor::FixParameters ()
{
	val[6] = 1.;
	val[9] = 1.;
	std::cout << "> Parameters val[6], val[9] fixed!" << std::endl;
}
...
TComplex FFactor::ScalarOne (TComplex t)
{
	...
	suma = normA*(nor[0]*mul[2]*mul[3] +
	  (mul[0]*mul[3]*(sub[3]-sub[0])/(sub[3]-sub[2]) -
	   mul[0]*mul[2]*(sub[2]-sub[0])/(sub[3]-sub[2]) -
	   mul[2]*mul[3])*a[4] +
	  (mul[1]*mul[3]*(sub[3]-sub[1])/(sub[3]-sub[2]) -
	   mul[1]*mul[2]*(sub[2]-sub[1])/(sub[3]-sub[2]) -
	   mul[2]*mul[3])*a[5]);
	return suma;
}
