 ...
 *
 * @file
 * @brief	Plot of nucleon form factors.
 */
...
void performPlot ( char* p, char* f ) {
	...
	for (int i = 0; i < nPoints; ++i) {
		...
		*/
		...
		plotY2[i] = nPlot.AbsGEN(z);
		plotY3[i] = nPlot.AbsGMN(z);

    }
	
	int k0 = 0 , k1 = 0, k2 = 0, k3 = 0;
	for (int i = 0; i < W.size(); i++) {
		//std::cout << W.type[i] << ": " << W.x[i] << " " << W.val[i] << std::endl;		
		// "protonElectric"
		if ((type[i] == 1) || (type[i] == 2)) {
			...
			dataD0[k0] = errDown[i];
			dataU0[k0] = errUp[i];
			//std::cout << i << ": " << W.x[i] << " " << W.val[i] << std::endl;
			++k0;
		}
		// "protonMagnetic"
		...
		if ((type[i] == 5) || (type[i] == 6)) {
			...
			dataD2[k1] = errDown[i];
			dataU2[k1] = errUp[i];
			//std::cout << type[i] << " " << k2 << ". " << dataX2[k2] << " " << dataY2[k2] << std::endl;
			++k2;
		}
		// "neutronMagnetic"
	...
	//graf6.viewPlusDataAE(nPoints,plotX,plotY1,k1,dataX1,dataY1,0,0,dataD1,dataU1,title);
	title = "|G_{E}^{n}|";
	//graf3.viewPlusData(nPoints,plotX,plotY2,k2,dataX2,dataY2,title);
	//graf7.viewPlusDataAE(nPoints,plotX,plotY2,k2,dataX2,dataY2,0,0,dataD2,dataU2,title);
	title = "|G_{M}^{n}|";
	//graf4.viewPlusData(nPoints,plotX,plotY3,k3,dataX3,dataY3,title);
	//graf8.viewPlusDataAE(nPoints,plotX,plotY3,k3,dataX3,dataY3,0,0,dataD3,dataU3,title);
	...
	graf9.viewPlusDataAE(nPoints,plotRX,plotRpY,numE,dataX4,dataY4,0,0,dataU4,dataD4,titleR);

	titleR = "mu_n*G_E^n/G_M^n";
	graf9.viewPlusDataAE(nPoints,plotRX,plotRnY,numF,dataX5,dataY5,0,0,dataU5,dataD5,titleR);
